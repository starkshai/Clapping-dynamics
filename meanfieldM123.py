...
for i in range(len(dy)):
    if dy[i]+expstartout[i]>1:
        ...
        dyu.append(dy[i])
err_range=[dy,dyu]
def fitfunc(t,lamb2,lamb3):
    def ode1(s, t):
        return N*lamb2* s * (1-s) +N*N*lamb3* s *s* (1-s)

    s0 = expstartout[0]
    ssol = odeint(ode1, s0, t)
    return ssol[:,0]
lamb_fit, lambcov = curve_fit(fitfunc, expstartTout, expstartout,p0=[0.001,0.0001],bounds=(0, [10.,10.]))
...
plt.legend(loc='best')
plt.savefig('M23fit.png')

def fitfunc12(t,lamb1,lamb2,lamb3):
    def ode2(s, t):
        return lamb1*(1-s)+N*lamb2* s * (1-s) +N*N*lamb3* s *s* (1-s)

    s0 = expstartout[0]
    ssol = odeint(ode2, s0, t)
    return ssol[:,0]
lamb_fit, lambcov = curve_fit(fitfunc12, expstartTout, expstartout,p0=[0.1,0.001,0.0001],bounds=(0, [10.,10.,10.]))
perr = np.sqrt(np.diag(lambcov))
print(lamb_fit)
print(perr)
tfit = np.linspace(0,int(expstartTout[-1])+1);
fit = fitfunc12(tfit, lamb_fit[0],lamb_fit[1],lamb_fit[2])
plt.figure()
plt.errorbar(expstarttimearr,expstartarr,yerr=err_range,fmt='o',ecolor='r',color='k',elinewidth=2,capsize=4,label='data')
plt.plot(tfit, fit, 'b-', label='fit')
plt.legend(loc='best')
plt.savefig('M123fit.png')